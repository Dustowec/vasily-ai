# ruff: noqa: E402
"""Streamlit дашборд для Vasily AI (финальная версия + ДИАГНОСТИКА)."""
...
if "agent" not in st.session_state:
    ...
    # Запускаем Watchdog вручную (он обычно стартует в agent.run())
    if config.watchdog_enabled:
        agent.watchdog = Watchdog(
            agent=agent,
            check_interval=config.watchdog_check_interval,
            restart_timeout=config.watchdog_restart_timeout,
            max_restarts=config.watchdog_max_restarts,
        )
        loop.run_until_complete(agent.watchdog.start())
        print("[Dashboard] Watchdog started")
    else:
        print("[Dashboard] Watchdog disabled in config")
...
def refresh_status():
    ...
    # ДИАГНОСТИКА: статус watchdog
    watchdog_status = None
    if agent and hasattr(agent, "watchdog") and agent.watchdog:
        try:
            watchdog_status = agent.watchdog.get_status()
            st.session_state.last_status = watchdog_status
        except Exception as e:
            st.session_state.last_status = {"error": str(e)}
    else:
        st.session_state.last_status = {"error": "Watchdog is None or not initialized"}

    # ДИАГНОСТИКА: контекст памяти
...
with col3:
    ...
    st.caption(f"DEBUG: last_status = {status}")

    if status and isinstance(status, dict):
        # Если есть ошибка — показываем её
        if "error" in status:
            st.error(f"Ошибка получения статуса: {status['error']}")
        elif status.get("llm"):
            ...
            st.info("Статус не загружен (данные получены, но нет ключа 'llm')")
            st.code(f"Содержимое status: {status}")
    else:
        st.info("Статус не загружен (status пустой или не словарь)")
