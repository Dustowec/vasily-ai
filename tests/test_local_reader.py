...
async def test_execute_file_not_found(tool, workspace_dir, monkeypatch):
    """When file doesn't exist, _execute returns error."""
    monkeypatch.chdir(workspace_dir.parent.parent)
    result = await tool._execute(path="workspace/reading/nonexistent.txt")
    assert result["status"] == "error"
    assert result["error_type"] == "invalid_url"
...
async def test_execute_file_too_large(tool, workspace_dir, monkeypatch):
    ...
    result = await tool._execute(path="workspace/reading/large.txt")
    assert result["status"] == "error"
    assert result["error_type"] == "invalid_url"
    assert "too large" in result["message"]


async def test_execute_unsupported_format(tool, workspace_dir, monkeypatch):
    """When file format is unsupported, _execute returns error."""
    monkeypatch.chdir(workspace_dir.parent.parent)
    unsupported = workspace_dir / "test.xyz"
    unsupported.write_text("test")
    result = await tool._execute(path="workspace/reading/test.xyz")
    assert result["status"] == "error"
    assert result["error_type"] == "invalid_url"
...
async def test_read_md(tool, workspace_dir, monkeypatch):
    """Read a .md file."""
    monkeypatch.chdir(workspace_dir.parent.parent)
    md_file = workspace_dir / "test.md"
    md_file.write_text("# Heading\n\nContent")
    result = await tool._execute(path="workspace/reading/test.md")
    assert result["status"] == "success"
    assert result["parser"] == "text"
...
# ==================== TEST JSON PARSER ====================
async def test_read_json(tool, workspace_dir, monkeypatch):
    """Read a .json file."""
    monkeypatch.chdir(workspace_dir.parent.parent)
    json_file = workspace_dir / "test.json"
    json_file.write_text(json.dumps({"key": "value", "number": 42}))
    result = await tool._execute(path="workspace/reading/test.json")
    assert result["status"] == "success"
...
# ==================== TEST CSV PARSER ====================
async def test_read_csv(tool, workspace_dir, monkeypatch):
    """Read a .csv file."""
    monkeypatch.chdir(workspace_dir.parent.parent)
    csv_file = workspace_dir / "test.csv"
    csv_file.write_text("name,age\nAlex,30\nBob,25")
    result = await tool._execute(path="workspace/reading/test.csv")
    assert result["status"] == "success"
...
# ==================== TEST XLSX PARSER ====================
async def test_read_xlsx_missing_dependency(tool, workspace_dir, monkeypatch):
    """When openpyxl is not installed, return error."""
    monkeypatch.chdir(workspace_dir.parent.parent)
    xlsx_file = workspace_dir / "test.xlsx"
    xlsx_file.write_text("dummy")
    with patch.dict(sys.modules, {"openpyxl": None}):
        result = await tool._execute(path="workspace/reading/test.xlsx")
        assert result["status"] == "success"
        assert "openpyxl is not installed" in result["content"]["error"]


# ==================== TEST PDF PARSER ====================
async def test_read_pdf_missing_dependency(tool, workspace_dir, monkeypatch):
    """When PyPDF2 is not installed, return error."""
    monkeypatch.chdir(workspace_dir.parent.parent)
    pdf_file = workspace_dir / "test.pdf"
    pdf_file.write_text("dummy")
    with patch.dict(sys.modules, {"PyPDF2": None}):
        result = await tool._execute(path="workspace/reading/test.pdf")
        assert result["status"] == "success"
...
def test_is_path_allowed_invalid(tool, tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    assert tool._is_path_allowed("workspace/reading/../../etc/passwd") is False
